...
class Duitku_CIMB extends WC_Payment_Gateway {
    protected $logger;
    protected $settings;

    public function __construct() {
        $this->id = 'duitku_cimb';
        $this->method_title = 'Duitku - CIMB Niaga Virtual Account';
        $this->method_description = 'Pembayaran melalui Virtual Account CIMB Niaga';
        $this->has_fields = false;
        $this->payment_code = 'B1'; // CIMB Niaga payment code

        // Load settings
        $this->settings = get_option('duitku_settings');
        $this->enabled = 'yes';
        $this->title = 'CIMB Niaga Virtual Account';
        $this->description = sprintf(
            'Pembayaran menggunakan Virtual Account CIMB Niaga. Expired dalam %d menit.',
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type' => 'checkbox',
                'label' => 'Enable CIMB Niaga Virtual Account Payment',
                'default' => 'yes'
            ),
            'title' => array(
    ...
    public function process_payment($order_id) {
        ...
        try {
            ...
            // Update order status
            $order->update_status('pending', __('Awaiting CIMB Niaga Virtual Account payment', 'duitku'));
            $order->save();
    ...
    public function receipt_page($order_id) {
        ...
        if ($va_number) {
            ...
            echo '<h3>' . esc_html__('CIMB Niaga Virtual Account Number', 'duitku') . '</h3>';
