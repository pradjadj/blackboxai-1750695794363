...
class Duitku_NobuBank extends WC_Payment_Gateway {
    protected $logger;
    public $settings;

    public function __construct() {
        $this->id = 'duitku_nobubank';
        $this->method_title = 'Duitku - QRIS NobuBank';
        $this->method_description = 'Pembayaran melalui QRIS NobuBank';
        $this->has_fields = false;
        $this->payment_code = 'DQ'; // NobuBank QRIS payment code

        // Load settings
        $this->settings = get_option('duitku_settings');
        $this->enabled = 'yes';
        $this->title = 'QRIS NobuBank';
        $this->description = sprintf(
            'Pembayaran menggunakan QRIS NobuBank. Expired dalam %d menit.',
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type' => 'checkbox',
                'label' => 'Enable QRIS NobuBank Payment',
                'default' => 'yes'
            ),
            'title' => array(
    ...
    public function process_payment($order_id) {
        ...
        try {
            ...
            // Update order status
            $order->update_status('pending', __('Awaiting QRIS NobuBank payment', 'duitku'));
            ...
            $this->logger->log('QRIS NobuBank Payment processing failed: ' . $e->getMessage());
    ...
    public function receipt_page($order_id) {
        ...
        if ($qr_string) {
            ...
            echo '<ol>';
            echo '<li>' . esc_html__('Buka aplikasi NobuBank di smartphone Anda', 'duitku') . '</li>';
            echo '<li>' . esc_html__('Pilih menu Scan QRIS', 'duitku') . '</li>';
            echo '<li>' . esc_html__('Scan QR Code di atas', 'duitku') . '</li>';
            echo '<li>' . esc_html__('Periksa detail transaksi dan lakukan pembayaran', 'duitku') . '</li>';
            echo '</ol>';
            echo '</div>';
        }
